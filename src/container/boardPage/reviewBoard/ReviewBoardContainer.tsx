...
function ReviewBoardContainer() {
  ...
  return (
    <Wrapper>
        ...
        {status === "loading" && <ToastMessage wrapType='none' messageString='불러오는 중...' timer={5000}/>}
        {status === "error" && <p>error</p>}
        {status === "success" && resData.length !== 0 && <Contents data={resData} />}
        {status === "success" && toast && <ToastMessage wrapType='none' messageString='더이상 불러올 리뷰 정보가 없습니다.' timer={5000}/>}
