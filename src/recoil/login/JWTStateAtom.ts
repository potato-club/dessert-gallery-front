...
const { persistAtom } = recoilPersist({
  key: "JWTSessionStorage", //원하는 key 값 입력
  storage: sessionStorage,
});

const defaultValue: JWTDataType = {
  accessToken: "",
  refreshToken: "",
};

// 리코일 atom 선언, effects_UNSTABLE 속성을 이용해 웹스토리지 사용 정의
export const JWTStateAtom = atom<JWTDataType>({
  key: "JWTDataState",
...
export function useJWTState() {
  ...
  const [value, setValue] = useRecoilState(JWTStateAtom);
